    ...
    <>
        ...
        >
          ...
          <span
            className={`flex items-center justify-center shrink-0 transition-all duration-500 ${
              isCollapsed ? "p-1" : "-ms-1 me-3"
            }`}
            style={{ transitionTimingFunction: softSpringEasing }}
          >
            <SearchIcon
              className={`text-muted-foreground/80 ${isCollapsed ? "ml-6" : ""}`}
              size={16}
              aria-hidden="true"
            />
          </span>
          ...
          <span
            className={`flex-1 min-w-0 transition-[opacity,width,margin] duration-500 overflow-hidden text-left ${
              isCollapsed ? "opacity-0 w-0 m-0" : "opacity-100 w-auto"
            }`}
            style={{ transitionTimingFunction: softSpringEasing }}
          >
          ...
          <kbd
            className={`bg-background text-muted-foreground/70 inline-flex h-5 items-center rounded border px-1 text-[0.625rem] font-medium transition-opacity duration-500 ${
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          {/* Sidebar + site routes */}
          {groupBy(searchData, (i) => i.group ?? "Navigation").map(
            ([group, rows]) => (
              <CommandGroup key={group} heading={group}>
                {rows.map((row) => (
                  <CommandItem
                    key={row.id}
                    value={[row.title, row.subtitle, row.keywords?.join(" ")]
                      .filter(Boolean)
                      .join(" ")}
                    onSelect={() => {
                      setOpen(false);
